...
.dash-table .rank { color: #444; font-size: 0.75rem; }
.dash-table .name { color: #eee; font-weight: 500; white-space: nowrap; max-width: 220px; overflow: hidden; text-overflow: ellipsis; }
.score-bar-wrap { width: 50px; background: #1a1a1a; border-radius: 3px; height: 6px; display: inline-block; vertical-align: middle; margin-right: 6px; }
.score-bar-fill { height: 6px; border-radius: 3px; background: linear-gradient(90deg, #1e3a5f, #3b82f6); }
.score-val { vertical-align: middle; color: #3b82f6; font-weight: 600; }
.dash-table th:last-child, .dash-table td:last-child { min-width: 120px; }
</style>
...
if filtered.empty:
    st.warning("没有符合条件的数据，请在左侧重新选择品牌。")
    st.stop()
...
with col_left:
    ...
    table_html = f"""
    <div style="overflow-x:auto; border:1px solid #222; border-radius:8px; max-height:460px; overflow-y:auto; background:#111;">
...
with col_right:
    st.markdown('<div class="section-title">📊 Top 10 综合得分</div>', unsafe_allow_html=True)
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            range=[0, top10["total_score"].max() * 1.18],
        ),
